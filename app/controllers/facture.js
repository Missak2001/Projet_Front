...
class FactureController extends BaseFormController {
    ...
    async registerFacture() {
        ...
        let categorie_f = this.validateRequiredField('#fieldCategorie', 'Catégorie');
        let statut = this.validateRequiredField('#fieldStatut', 'Statut');
        let adresse_facturation = this.validateRequiredField('#fieldAdresseFacturation', 'Adresse de Facturation');
    ...
    updateTotalPrice() {
        ...
        document.getElementById('fieldPrix').value = totalPrice.toFixed(2);
        document.getElementById('fieldPrixTTC').value = prixTTC.toFixed(2);
    }
}
